...
def create_alphabets(alphabet_directory, train_path, data_paths=None, transfer_train_path=None,
                     transfer_data_paths=None, transfer=True,
                ...
                for data_path in transfer_data_paths:
                    ...
                    with open(data_path, 'r') as file:
                        for line in file:
                            ...
                            pos = tokens[2]
                            trigger = tokens[3]
                            transfer_tag = tokens[4]
                            #pos_alphabet.add(pos)
                            #chunk_alphabet.add(trigger)
                            transfer_alphabet.add(transfer_tag)
    ...
    srl_alphabet = Alphabet('srl')
    transfer_alphabet =  Alphabet('pb') # this the alphabets of the
                                        # downstream task to be transfer learned
    ...
    if not os.path.isdir(alphabet_directory):
        ...
        srl_alphabet.add(PAD_SRL)
        transfer_alphabet.add(PAD_TRANSFER)
        ...
        with open(train_path, 'r') as file:
            for line in file:
                ...
                if word in vocab:
                    ...
                    vocab[word] = 1
        if transfer:
            with open(transfer_train_path, 'r') as file:
                for line in file:
                    line = line.decode('utf-8')
                    line = line.strip()
                    if len(line) == 0:
        ...
        srl_alphabet.save(alphabet_directory)
        transfer_alphabet.save(alphabet_directory)
    else:
        word_alphabet.load(alphabet_directory)
        char_alphabet.load(alphabet_directory)
        pos_alphabet.load(alphabet_directory)
        chunk_alphabet.load(alphabet_directory)
        srl_alphabet.load(alphabet_directory)
        transfer_alphabet.load(alphabet_directory)
    ...
    return word_alphabet, char_alphabet, pos_alphabet, chunk_alphabet, srl_alphabet, transfer_alphabet


def read_data(source_path, word_alphabet, char_alphabet, pos_alphabet, chunk_alphabet, srl_alphabet, max_size=None,
              normalize_digits=True):
    data = [[] for _ in _buckets]
    max_char_length = [0 for _ in _buckets]
    print('Reading data from %s' % source_path)
    counter = 0
    reader = CoNLL03Reader(source_path, word_alphabet, char_alphabet, pos_alphabet, chunk_alphabet, srl_alphabet)
...
def read_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, chunk_alphabet, srl_alphabet,
    ...
    for bucket_id in range(len(_buckets)):
        ...
        srls = Variable(torch.from_numpy(nid_inputs), volatile=volatile)
        ...
        data_variable.append((words, chars, pos, chunks, srls, masks, single, lengths))
...
def get_batch_variable(data, batch_size, unk_replace=0.):
    ...
    words, chars, pos, chunks, srls, masks, single, lengths = data_variable[bucket_id]
    bucket_size = bucket_sizes[bucket_id]
    batch_size = min(bucket_size, batch_size)
    ...
    return words, chars[index], pos[index], chunks[index], srls[index], masks[index], lengths[index]
    ...
    for bucket_id in bucket_indices:
        ...
        for start_idx in range(0, bucket_size, batch_size):
            if shuffle:
                ...
                excerpt = slice(start_idx, start_idx + batch_size)
            yield words[excerpt], chars[excerpt], pos[excerpt], chunks[excerpt], srls[excerpt], \
